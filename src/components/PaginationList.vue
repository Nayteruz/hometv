<template>
  <div class="pagination">
    <ul>
      <li class="pg first" v-show="filmStore.pageNum > 1" @click="emitPage(null, 'prevAll')"><span><IconArrowBig/></span>
      </li>
      <li class="pg prev" v-show="filmStore.pageNum > 1" @click="emitPage(null, 'prev')"><span><IconArrowSmall/></span>
      </li>
      <li v-for="num in getList(total)" :key="num">
        <span :class="{'active':filmStore.pageNum === num}" @click="emitPage(num)">{{ num }}</span>
      </li>
      <li class="pg next" v-show="filmStore.pageNum < props.total" @click="emitPage(null, 'next')"><span><IconArrowSmall/></span>
      </li>
      <li class="pg last" v-show="filmStore.pageNum < props.total" @click="emitPage(null, 'nextAll')"><span><IconArrowBig/></span>
      </li>
    </ul>
  </div>
</template>
